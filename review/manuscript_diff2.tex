...
%DIF DEL manuscript_old2.tex   Wed Aug 24 15:12:57 2022
%DIF ADD manuscript.tex        Sat Sep 24 16:55:18 2022
...
recoil length, $\Delta l$\DIFaddbegin \DIFadd{, at the point of the joint}\DIFaddend . For $n$ hairs in contact, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{when a hair detaches, 
the array's tilt angle will change from initial $\theta_{b}$ to $\theta_{a}$ as a result of joint rotation. 
The $i^{th}$ hair of the array thus deforms by $\Delta l_{i}$, which by geometry, is derived as:
}\begin{equation}
\DIFadd{\Delta l_{i}=\Delta l + iw\left(\tan \theta_{a}-\tan \theta_{b}\right)\label{eq:delta_li}
}\end{equation}

 \DIFadd{The }\DIFaddend force balance before and after a hair detaches is \DIFaddbegin \DIFadd{thus }\DIFaddend given respectively by:

\[
\DIFdelbegin %DIFDELCMD < \begin{array}{c}
%DIFDELCMD < \stackrel[i=1]{n}{\sum}k_{h}\left(l_{h,i}-l_{h,0}\right)=k_{l}\left(l_{l}-l_{l,0}\right)\\
%DIFDELCMD < \stackrel[i=1]{n-1}{\sum}k_{h}\left(l_{h,i}+\Delta l-l_{h,0}\right)=k_{l}\left(l_{l}-\Delta l-l_{l,0}\right)
%DIFDELCMD < \end{array}%%%
\DIFdelend \DIFaddbegin \begin{array}{c}
\stackrel[i=1]{n}{\sum}k_{h}\left(l_{h,i}-l_{h,0}\right)=k_{l}\left(l_{l}-l_{l,0}\right)\\
\stackrel[i=1]{n-1}{\sum}k_{h}\left(l_{h,i}+\Delta l_{i}-l_{h,0}\right)=k_{l}\left(l_{l}-\Delta l-l_{l,0}\right)
\end{array}\DIFaddend 
\]

Solving the above two equations \DIFaddbegin \DIFadd{with equation \ref{eq:delta_li} }\DIFaddend for $\Delta l$, we get:

\begin{equation}
\Delta l=\DIFdelbegin \DIFdel{\frac{f_{p}}{k_{h}\left(n-1\right)+k_{l}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{f_{p}-k_{h}w(n/2)(n-1)\left(\tan \theta_{a}-\tan \theta_{b}\right)}{k_{h}\left(n-1\right)+k_{l}}}\DIFaddend \label{eq:backing_deform}
\end{equation}
...
\[
\hat{f}_{p}=\frac{f_{p}}{k_{h}w},\quad\hat{F}_{net}=\frac{F_{net}}{k_{h}w},\quad\hat{d_{s}}=\frac{d_{s}-l_{h,0}}{w},\quad\hat{s}=\frac{s}{L}
\]
...
force values represent attraction by convention. \DIFaddbegin \DIFadd{Python scripts were written to solve the above equations under specific detachment conditions (available in the public repository }\url{https://github.com/PranavSudersan/paper-effect_of_tilt/blob/main/codes/Fibrillar\%20adhesion\%20-\%20Effect\%20of\%20tilt.ipynb}\DIFadd{)
}\DIFaddend 
...
curves for such a scenario. \DIFaddbegin \DIFadd{To summarize the numerical procedure in brief, we start with an initial $n=25$ hairs. ${F}_{net}$ is then calculated for increasing ${d_{s}}$, until $d_{s}= d_{s,max}$, upon which $n$ value is decremented by 1. The above process is repeated for increasing $d_{s}$ until $n=0$, indicating complete detachment. For the case of the soft leg, on each instance when $n$ is decremented,  ${d_{s}}$ is increased by an additional $\Delta l$ value (equation \ref{eq:backing_deform}) in order to account for the recoil effect of the leg.
}\DIFaddend 
...
jitter in the force curves. The \DIFdelbegin \DIFdel{hairs of the }\DIFdelend pad with a higher tilt
angle \DIFdelbegin \DIFdel{will start to detach first, followed by the ones }\DIFdelend \DIFaddbegin \DIFadd{initiates hair detachment first, in comparison to a pad }\DIFaddend with a lower
tilt. 
...
contact if $\theta$ is kept constant \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(i.e. $\theta_{b}=\theta_{a}$). }\DIFaddend This implies that, once initiated,
...
curves during a free vertical pull of the adhesive pad. 
\DIFaddbegin \DIFadd{A similar numerical procedure as }\emph{\DIFadd{fixed pull}} \DIFadd{is followed here. }\DIFaddend Since the
...
As the hairs detach, the array gets tilted more and more \DIFaddbegin \DIFadd{(i.e. $\theta_{b}<\theta_{a}$)}\DIFaddend , making it
less likely for the recoil length, $\Delta l$, to exceed \DIFdelbegin \DIFdel{$w\sin\theta$
}\DIFdelend \DIFaddbegin \DIFadd{$w\sin\theta_{a}$
}\DIFaddend and detach the next adjacent hair. Thus here, we don't see any propagation
...
fix $s_{l}/L=1$ \DIFaddbegin \DIFadd{(here, $L=24w$) }\DIFaddend and $s_{h}/w=10$ and vary the vertical claw-hinge
...
implying that the claw should \DIFdelbegin \DIFdel{adhere }\DIFdelend \DIFaddbegin \DIFadd{stick }\DIFaddend well with the surface\DIFaddbegin \DIFadd{, perhaps by mechanical interlocking, }\DIFaddend to resist
...
the number of hairs, $N_{t}=25$. The dimensionless parameters $\hat{d_{s}}=\frac{d_{s}-l_{h,0}}{w}$ and 
$\hat{s}=\frac{s}{L}$, as described in text. \label{fig:Reduction-factor_fp}}
\end{figure}
...
This highlights \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{another }\DIFaddend advantage of having a split contact design found
in many biological systems. A \DIFdelbegin \DIFdel{higher }\DIFdelend \DIFaddbegin \DIFadd{design comprising of a large }\DIFaddend number of hairs \DIFdelbegin \DIFdel{offers }\DIFdelend \DIFaddbegin \DIFadd{not only enhances the adhesion due to scaling effects \mbox{%DIFAUXCMD
\cite{Kamperman2010}}\hskip0pt%DIFAUXCMD
, but could also offer }\DIFaddend a better
control over adhesion\DIFdelbegin \DIFdel{and thus is more suited }\DIFdelend \DIFaddbegin \DIFadd{, making it quite suitable }\DIFaddend for reversible attachment
...
dimensionless parameter, $\hat{f}_{p}=0.1$. The dimensionless parameters $\hat{d_{s}}=\frac{d_{s}-l_{h,0}}{w}$ and $\hat{s}=\frac{s}{L}$, as described in text. \label{fig:Reduction-factor-Nt}}
\par\end{centering}
\end{figure}

Figures \ref{fig:Reduction-factor_fp} and \ref{fig:Reduction-factor-Nt} can be combined into a single set of colour plots by defining a new dimensionless parameter,  $\chi=\hat{f}_{p} N_{t}=\frac{f_{p}N_{t}}{k_{h}w}$ (see Appendix A). Overall, we see that \emph{flex} mode of detachment shows the highest reduction factor among the three modes, with the optimal value of $\chi \sim 1$.
...
pull} (similar to path 3 above). \DIFaddbegin \DIFadd{Lateral video recordings showed that in this scenario, claws were the last to detach when a leg loses contact with the surface, which also indicates a }\emph{\DIFadd{flex}} \DIFadd{mode of detachment (similar to path 5). }\DIFaddend 3) Only a fraction of the beetle's
...
effective modulus. Buckling could also, interestingly, promote easier detachment in the \emph{free pull} mode.  
...
crucial parameters to consider in the design of an optimal array system which shows reversible adhesion via \emph{elastic weakening}.
...
\DIFdelbegin \DIFdel{Inhomogeneous deformation of hairs results in significant reductions
in the adhesion of }\DIFdelend \DIFaddbegin \DIFadd{Controlled detachment of }\DIFaddend a fibrillar system similar to an insect's tarsal hairy adhesive pads \DIFdelbegin \DIFdel{. Such a condition }\DIFdelend can be achieved
...
the pad around the claw. \DIFaddbegin \DIFadd{In all three scenarios, an inhomogeneous deformation of hairs across the array results in significant reductions
in the net adhesion due to an elastic effect. }\DIFaddend Strategic control of the joint's mobility or claw
...
The proposed model \DIFdelbegin \DIFdel{is supported by }\DIFdelend \DIFaddbegin \DIFadd{has been compared with }\DIFaddend previously reported experimental
...
\section{{Reduction factor: master plot}}
...
\begin{figure}[H]
\noindent \begin{centering}
\includegraphics{Reduction_factor-chi}\caption{\textbf{Effect of $\chi$ on reduction factor.} 
Here, we define a unified dimensionless design parameter $\chi=\frac{f_{p}N_{t}}{k_{h}w}$, combining $\hat{f}_{p}$ and $N_{t}$ into a single number. The dimensionless parameters $\hat{d_{s}}=\frac{d_{s}-l_{h,0}}{w}$ and $\hat{s}=\frac{s}{L}$, as described in text. \label{fig:Reduction-factor-chi}}
\par\end{centering}
\end{figure}
