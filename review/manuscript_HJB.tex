...
of the hairs. In this study, we propose possible mechanisms by which
a hairy pad can be easily detached even when the hairs possess no
asymmetry. Based on a spring-based model, we consider three modes
of detachment: vertically pulling the pad while maintaining either
a 1) fixed or a 2) free leg joint, or by 3) flexing the pad about
...
Our proposed model illustrates the design advantage provided by hairy
adhesive systems, which not only provides strong adhesion, but also
allows easy detachment, making it suitable as organs for fast locomotion.
The presented approaches can be potentially used to switch the adhesion
state in bio-inspired artificial systems without the need of asymmetric
structures.
\end{abstract}

\section{Introduction}

Over the past few decades, there has been numerous studies to understand
how animals such as geckos and insects are able to walk on surfaces
of any direction while defying gravity. A microscopic observation
reveals that, in many cases, animals have a dense array of hairy structures
at the end of their legs \cite{RN198,RN59}. These hairy adhesive
pads help the animal to stay attached to 
surfaces or detach easily
...
hairy pads\cite{RN40,RN120} has followed two fundamental approaches:
either by energy balance, or by force balance. 

In the energy balance approach, adhesion is characterized by ``work
...
energy stored in the hair is dissipated, which increases W\textsubscript{adh}
and thus adhesion is enhanced\cite{RN29,RN114}. 
% I do not understand the last sentence. What precisely does "`adhesion is enhanced"' mean? If the work of adhesion is fixed, how can it be adhanced? 
Detachment of an
individual hair can be explained based on Kendall's peeling theory\cite{RN147},
which predicts low forces at high peeling angles.
% The last sentence seems to belonmg more to the next paragraph. 

In the force balance approach, adhesion is characterized by a pull-off
...
contact. Based on a ``cohesive zone model'', Hui et. al.\cite{RN72}
had identified two regimes of single hair detachment: 1) a flaw sensitive
regime, where contact failure occurs due to crack propagation leading
to low \textgreek{sv}\textsubscript{p}, 2) a flaw insensitive regime,
where the contact interface fails simultaneously, leading to high
\textgreek{sv}\textsubscript{p}. Tian et. al. \cite{RN230} had shown
...
and controlling the pulling angle to disorient the hairs. Federle\cite{RN20}
had further argued that the curved shape of the hair helps the pad
to stay attached when pulled proximally, and easily detached by elastic
recoil when pushed distally. 

% I suggest to describe both models a bit more in detail. What is a flaw sensitive regime? What does "`flaw"' mean and how is it related to adhesion? 

The theories presented so far suggests that low detachment force of
a hairy pad can be attained by 1) increasing the stress concentration
by peeling the pad at high angles, or 2) laterally shearing the pad
...
possess flat ``mushroom'' tipped hairs on their pads\cite{RN19}.
They are relatively symmetric compared to the ``spatula''
tipped hairs. This begs the question of how the beetle can still easily
detach their legs during locomotion, since these mushroom shaped hairs
should be resilient to detachment via lateral shearing. Artificial
micro-pillar adhesives mimicking the mushroom shaped hairs of the
beetle have in fact demonstrated its superior adhesive strength in
comparison to other geometries\cite{RN112,RN57}. Introducing asymmetry
into the pillar geometry of such mimics for easy detachment is however
challenging due to current limitations in fabrication techniques and
difficulty in scaling-up for practical applications \cite{RN57}.
Alternate strategies are thus desired to switch the adhesion state
of symmetric pillar arrays in a reversible manner, e.g. by buckling
...
In this work, we propose a mechanisms by which adhesive
pads with symmetric hairs can easily detached from a surface. 
The hairy pad is modeled as an array of springs and the
force-balance approach is used to quantify the net adhesion \cite{RN117}.
We hope our work to provide strategies to control the adhesion force
of an artificial micro-pillar adhesive, which has applications in
bio-inspired climbing robots, pick-and-place operations and reusable
adhesives.

\section{Model}

The hairy pad is assumed to be a one dimensional array of $N_{t}$
...
deformable leg, assumed to be another spring with spring constant,
$k_{l}$, and natural length, $l_{l,0}$. The leg is hinged to the
array at a distance, $s$, from the right end of the array. The hinge
...
The pad is oriented at an angle, $\theta$, while making contact with
...
$F_{net}$ is the net force on the pad and $M_{net}$ is the the net
torque about the hinge, at a particular instant during the detachment
process.
% Is the angle at the hinge fixed? What happens, when I start pulling vertically?


\begin{figure}[H]
\includegraphics{Figure1-model_schematic}\caption{Spring contact model of a hairy adhesive pad. The pad consists of
an array of $N_{t}$ hairs connected to a deformable leg at the hinge.
...
Simplifying, we get (see Appendix section for derivation):
...
The net torque, $M_{net}$, about the hinge can be similarly derived
(Appendix):
...
Let us now consider the scenario where even the leg above the hinge
...
We consider three possible mechanisms to detach the adhesive pad from
a surface: 1) \emph{Fixed pull}, where the pad is pulled vertically
up while keeping a fixed leg-hinge, 2) \emph{Free pull,} where the
pad is pulled vertically up while keeping the leg-hinge free to allow
rotation of the array and 3) \emph{Flex}, where the pad is hinged
...
detail, let us assume a pad with $N_{t}=25$ hairs and $\hat{f_{p}}=0.1$
attached to a stiff leg ($k_{l}\rightarrow\infty$). The situation
of a soft leg ($k_{l}/k_{h}=10$) is also considered for the first
two cases involving vertical detachment.

\subsection{Fixed pull}

\begin{figure}[H]
\includegraphics{Case1-Fixed_pull}\caption{Force distance curves for a hairy pad, pulled vertically upwards with
a fixed hinge between the pad and the leg (\emph{Fixed pull}). The
tilt angle, $\theta$, of the array is kept fixed during detachment.
The leg is either stiff ($k_{l}\rightarrow\infty$) or soft ($k_{l}/k_{h}=10$).
Positive force values represent attraction. The green dashed line
...
The hairy adhesive pad can be detached by pulling it vertically upwards
while maintaining a constant tilt angle, $\theta$, with the surface.
This can be achieved if the joint connecting the leg to the array
(leg-hinge) is kept fixed. Equations \ref{eq:force_n_d}, \ref{eq:d_max}
...
further reducing the net adhesive power of the array. We term this
...
deformation and ultimately detaches simultaneously after a distance,
$\hat{d_{s}}=0.1$. Here, no \emph{elastic weakening} occurs and the
pad shows the maximum possible adhesion.

For the case of a stiff leg, we see that at small distances, all hairs
...
For the case of a soft leg, we observe a similar effect of tilt angle
...
until the pad completely breaks contact with the surface.

\subsection{Free pull}

\begin{figure}[H]

\includegraphics{Case2-Free_pull}\caption{Force distance curves for a hairy pad, pulled vertically upwards with
a free hinge between the pad and the leg (\emph{Free pull}). $s$
is the distance between the leg-hinge and the right end of the array
and $L$ is the array length. The free hinge allows further tilting
of the array during the vertical pull. The leg is either stiff ($k_{l}\rightarrow\infty$)
...
However now, the leg-hinge is assumed to be free. In this case, the
array will reorient itself to maintain a zero net torque about the
leg-hinge during the entire detachment process. At any given instant,
...
position of the leg-hinge will influence the net torque, we use the
ratio, $s/L$, to study its effect on the detachment forces.

Maximum adhesion is seen when the leg-hinge is positioned at the centre
of the array, i.e. $s/L=0.5$ (Figure \ref{fig:Free-pull}). Here,
the net torque due to the hairs is balanced by symmetry and the array
remains parallel to the substrate until all hairs detach simultaneously
at $\hat{d_{s}}=0.1$. Shifting the position of the leg-hinge further
away from the array centre leads to lower forces or adhesion. The
resulting torque imbalance will tilt the array, which reduces the
net force due to the \emph{elastic weakening} effect, as described
in the previous section. Higher $s/L$ increases the net torque to
...
trend is seen for both a stiff and a soft leg. The elastic recoil
...
\begin{figure}[H]
\includegraphics{Case3-Fixed_flex}\caption{Force curves for a hairy pad detached by flexing it about the claw.
$\hat{F}_{pull}=\frac{F_{pull}}{k_{h}w}$ is the normalised pulling
force necessary to apply the torque about the claw-hinge for detachment,
...
a mode of detachment will be driven by a torque applied by the leg
...
end of the array; $s_{l}$ be the distance between the fixed leg-hinge
and the right end of the array. To illustrate the mechanism, let us
...
in a decrease in the net torque and thus lower $F_{pull}$ for smaller
...
on the hinge, $F_{claw-hinge}$, follows a similar trend as $F_{pull}$.
...
detachment (Figure \ref{fig:Reduction-factor_fp}). From the heat
maps, one can generally say that lower values of $\hat{f}_{p}$ increase
the reduction factor and thus provide a greater possibility of controlling
adhesion.

\begin{figure}[H]
\noindent \includegraphics{Reduction_factor-fp}\caption{Heat maps showing the effect of the dimensionless parameter, $\hat{f}_{p}$,
...
the position of the leg-hinge further away from the array (i.e. high
$s/L$) results in large values of $r$. In this scenario, the higher
torque exerted by the array leads to a higher tilt, and thus increases
...
detachment is driven by torque, the pulling force necessary to provide
the torque can be decreased without any limit simply by having a long
lever arm ($\hat{s}_{l}$). In contrast, for the previous cases of
\emph{free pull} and \emph{fixed pull}, the reduction factor is capped
to the maximum number of hairs in the array ($N_{t}=25$). Here, \emph{elastic
weakening} can only reduce the array's adhesion force from $N_{t}$
hairs down to a single hair.
...
The heat maps show that high $N_{t}$ increases $r$ irrespective
of the mode of detachment. Under a tilted state, more number of hairs
...
in many biological systems. Higher number of hairs offer a better
...
\includegraphics{Reduction_factor-Nt}\caption{Heat maps showing the effect of the number of hairs, $N_{t}$, on
...
one can think of several strategies to detach a hairy adhesive pad
...
pads in each of their legs, each possessing hairs of different geometries.
To keep our analysis simple, we will assume each leg to have only
two pads, with identical hairs of ``mushroom'' geometry. The distal
...
hairs, respectively, by combining the hairs along the width. Based
on reported measurements\cite{RN79}, the beetle's combined ``hair''
is thus considered to have an effective pull-off force, $f_{p}=0.5\times25=12.5$
\textgreek{m}N and spring constant, $k_{h}=0.5\times25=12.5$ N/m
. The beetle's hairs are approximately $l_{h,0}=40$ \textgreek{m}m
long, spaced $w=10$ \textgreek{m}m apart. The adhesive pad also has
a claw, around 200 \textgreek{m}m long, and the leg is connected roughly
at the end of the proximal pad. This will put $\hat{s}_{h}=20$ and
...
as a hinges for rotation (H\textsubscript{1} and H\textsubscript{2}
in Figure \ref{fig:Detachment-pathways} inset). The claw can be used
as an external hinge (H\textsubscript{3}) by flexing the pad around
it.
...
First, let us assume the hinge H\textsubscript{2} to be fixed, such
that both the distal and proximal pads can be combined to be a single
...
If the pad is detached by keeping H\textsubscript{1} fixed and maintaining
...
fixed, stretching the hairs. On reaching point $a$, H\textsubscript{1}
is set free, which results in a sudden drop in force due to the excess
torque by the stretched hairs, tilting the array. Beyond this, the
force curve follows the free pull mechanism, with $\thicksim3.5$
...
hinge H\textsubscript{2} to be free such that two pads can behave
...
\begin{figure}[H]
\includegraphics{Detachment_pathways}\caption{Force distance curves showing possible detachment pathways for a dock
beetle's leg, derived from the spring contact model. The curves are
offset laterally for clarity. Colours represent the distinct detachment
pathways, as labelled correspondingly, with arrows indicating the
direction of retraction. The different line style denotes the specific
detachment mechanism followed by any region of the pathway. The inset
schematic shows the assumed locations of the different hinges (H\protect\textsubscript{1},
...
and release of its leg joints can help the insect to reduce the pad's
...
beetles can modulate its adhesion by taking advantage of this effect.
...
provide support to our proposed theory: 1) The detachment was shown
...
weakening}. 2) The beetle can at times instantaneously detach all
its legs and ``drop'' itself while upside down. This could be explained
by the beetle freeing up its leg joints and using just its body weight
...
4) Contact images showed that the array ``peels'' from the proximal
...
be able to ``peel'' its array, since peeling, strictly speaking,
depends on the elastic contribution of a thin flexible backing as
it bends during the process\cite{RN147}. Rather, the ``peeling''
observed in the beetle's case should be a result of the pad detaching
from the surface in a tilted orientation, causing the hairs to detach
...
to its elastic limit, we can show from simple geometry, that, the
...
in the adhesion of a hairy pad system. Such a condition can be achieved
by either 1) pulling the pad while maintaining a constant tilt angle,
2) pulling the pad while maintaining a free leg joint or 3) flexing
the pad around the claw. Strategic control of the joint's mobility
...
The proposed theory is supported by previously reported experimental
observations of leg detachment in dock beetles and opens up new strategies
to achieve reversible adhesion in bio-inspired artificial adhesives
without the need of asymmetric structures.

\section{Acknowledgment}

We are grateful to Thomas Endlein, Ren\textcyr{\`\cyre} Hensel and
Bat-El Pinchasik for fruitful discussions. This work was funded by
\emph{Deutsche Forschungsgemeinschaft} (Grant number: PI 1351/2-1).
...
The net torque about the hinge due to the deformed hairs of the array
is,
