...
asymmetry. Here, we examine the possible function of the tibia-tarsus (t-t) leg joint and the claws.
Based on a spring-based model, we consider three modes
of detachment: vertically pulling the pad while maintaining either
a 1) fixed or a 2) free t-t leg joint, or by 3) flexing the pad about
...
Here, we focus our analysis to only normal adhesion force necessary to separate 
the pad from a flat surface.
...
array at a distance, $s$, from the right end of the array. The hinge, analogous to the tarsus-tibia (t-t) leg joint of an insect, 
...
process. The contact points between the hair and the surface is assumed to be laterally fixed due to high friction forces.
...
up while keeping a fixed hinge (t-t leg joint); 2) \emph{Free pull,} where the
pad is pulled vertically up while keeping the hinge (t-t leg joint) free to allow
...
attached to a stiff leg (tibia) with $k_{l}\rightarrow\infty$. The situation
of a soft leg (tibia) with $k_{l}/k_{h}=10$ is also considered for the first
...
a fixed hinge (tarsus-tibia leg joint). The
...
This can be achieved if the hinge (t-t leg joint) is kept fixed. 
...
Force distance curves for a fibrillar adhesive pad, pulled vertically upwards with
a free hinge (tarsus-tibia leg joint). $s$
