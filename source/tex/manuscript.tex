...
Insects such as flies or beetles have the ability to walk on any surface against gravity. It was previously shown that
these insects possess a dense array of micrometer sized hairs on their legs. These hairy legs are responsible for the 
insect's strong adhesion to surfaces by resisting contact failure due to micro-roughness or defects on the surface.
In this work, we theoretically show that, a hairy design of the leg not only provides strong adhesion, but can also allow easy detachment from a surface. This is achieved simply by modulating the leg's tilt relative to the surface with the aid of its leg joint or claws. Such a design can thus inspire artificial adhesives with switchable adhesion properties.
...
array at a distance, $s$, from the right end of the array. The hinge (or joint) 
...
torque about the joint, at a particular instant during the detachment
process.
...
The net torque, $M_{net}$, about the joint can be similarly derived
(see Appendix A):
...
up while keeping a fixed leg joint; 2) \emph{Free pull,} where the
pad is pulled vertically up while keeping the leg joint free to allow
...
\begin{figure}[H]
\includegraphics{Case1-Fixed_pull}\caption{\textbf{Detachment by fixed pull.} 
Force distance curves for a fibrillar adhesive pad, pulled vertically upwards with
a fixed joint between the pad and the leg (\emph{Fixed pull}). The
...
This can be achieved if the joint connecting the leg to the array is kept fixed. 
...
\includegraphics{Case2-Free_pull}\caption{\textbf{Detachment by free pull.} 
Force distance curves for a fibrillar adhesive pad, pulled vertically upwards with
a free joint between the pad and the leg (\emph{Free pull}). $s$
is the distance between the leg joint and the right end of the array
...
However now, the leg joint is assumed to be free. In this case, the
...
position of the leg joint will influence the net torque, we use the
ratio, $s/L$, to study its effect on the detachment forces.

Maximum adhesion is seen when the leg joint is positioned at the centre
...
\begin{figure}[H]
\includegraphics{Case3-Fixed_flex}\caption{\textbf{Detachment by flex.}
...
end of the array; $s_{l}$ be the distance between the fixed leg joint
...
detachment (Figure \ref{fig:Reduction-factor_fp}). From the colour plots, 
one can generally say that lower values of $\hat{f}_{p}$ increase
...
the position of the leg joint further away from the array (i.e. high
...
Force distance curves showing possible detachment pathways for a dock
...
weakening}, which is suited for easy detachment. 2) The beetle can at times instantaneously detach all
...
The net torque about the joint due to the deformed hairs of the array
is,
